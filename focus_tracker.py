...
def main():
    ...
    if not os.path.exists(model_path):
        sys.exit(print(f"Error: {model_path} missing. Download it from MediaPipe's model catalog."))
        
    # FIX: Using MediaPipe v2 Tasks configuration safely mapped to streaming/live video mode
    ...
    det = FaceLandmarker.create_from_options(options)
    cap = get_camera()
    ...
    while True:
        ...
        # FIX: For RunningMode.VIDEO, we must track and pass a monotonically increasing timestamp in milliseconds
        frame_timestamp_ms = int(time.time() * 1000)
        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        results = det.detect_for_video(mp_image, frame_timestamp_ms)
        ...
        box_ds = int(np.clip(sm_score, 0, 100))
        score_history.append(float(sm_score))
        if len(score_history) > 100:
            score_history.pop(0)
            
        # Telemetry Card Panel (Bottom-Right)
        ...
        cv2.putText(frame, f"Focus Index: {box_ds}%", (x1 + 12, y1 + 38), cv2.FONT_HERSHEY_SIMPLEX, 0.4, C_WHT, 1, cv2.LINE_AA)
        
        bar_x1, bar_y1 = x1 + 12, y1 + 46
        bar_x2, bar_y2 = x2 - 12, y1 + 54
        cv2.rectangle(frame, (bar_x1, bar_y1), (bar_x2, bar_y2), (40, 40, 40), -1)
        bw = int((bar_x2 - bar_x1) * (box_ds / 100.0))
        bc = C_GRN if box_ds >= 70 else (C_ORG if box_ds >= 30 else C_RED)
        ...
        # Live Waveform Graph HUD Panel (Top-Left)
        p_w, p_h = 210, 95
        px1, py1 = 15, 15
        ...
        cv2.putText(frame, f"Live Index: {box_ds}%", (px1 + 12, py1 + 38), cv2.FONT_HERSHEY_SIMPLEX, 0.4, C_WHT, 1, cv2.LINE_AA)
        ...
        if time.time() < cal_end:
            ...
            btw, bth = cv2.getTextSize(banner, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)[0]
